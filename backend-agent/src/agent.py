...
class ZerePyAgent:
    ...
    def _construct_system_prompt(self) -> str:
        ...
        if self._system_prompt is None:
            ...
            prompt_parts.extend(f"Your Name is {self.name}")

            prompt_parts.extend(self.bio)

            if self.rules:
                prompt_parts.append("\n This are the Rules you must follow Strictly:")
                prompt_parts.extend(f"- {rule}" for rule in self.rules)
    ...
    def prompt_llm(
        self, prompt: str, system_prompt: str = None, model_provider="groq"
    ) -> str:
        """Generate text using the configured LLM provider"""
        system_prompt = system_prompt or self._construct_system_prompt()

        promptTemplate = f"chat_history: {self.chat_history} user: {prompt}"
        self.chat_history.append(prompt)
        result = self.connection_manager.perform_action(
            connection_name=self.model_provider,
            action_name="generate-text",
            params=[promptTemplate, system_prompt],
        )
